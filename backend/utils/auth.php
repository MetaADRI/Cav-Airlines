...
function registerUser($full_name, $email, $password, $phone, $conn) {
    ...
    // Insert new user - using the schema from group2_cavair_db.sql
...
function updateLastLogin($user_id, $conn) {
    // The current group2_cavair_db schema doesn't have a last_login field
...
function requireLogin($redirect_url = '../frontend/login.html') {
    ...
    // Session timeout disabled: Only log out when user chooses logout option
    // checkSessionTimeout();
}
...
function updateUserProfile($user_id, $data, $conn) {
    ...
    $stmt->bind_param("ssi", $data['email'], $data['phone'], $user_id);
    return $stmt->execute();
}
...
// Auth logs functionality removed as requested
?>
